...
const Dashboard: React.FC = () => {
    ...
    api
      ...
      .then(response => {
        const appointmentsFormatted = response.data
          .map(appointment => {
            const parseISODate = parseISO(appointment.date);
            const hourFormatted = format(parseISODate, 'HH:mm');
            return { ...appointment, hourFormatted };
          })
          .sort(
            (appointmentA, appointmentB) =>
              parseISO(appointmentA.date).getTime() - parseISO(appointmentB.date).getTime(),
          );
  ...
  const selectedWeekdaysAsText = useMemo(() => {
      ...
      <Content>
        <Schedule>
          ...
          <p>
            ...
            <span>{selectedWeekdaysAsText}</span>
          </p>
          ...
          <Section>
            <strong>Manhã</strong>
            {morningAppointments.length === 0 && <p>Nenhum agendamento marcado</p>}
          ...
          <Section>
            <strong>Tarde</strong>
            {afternoonAppointments.length === 0 && <p>Nenhum agendamento marcado</p>}
